"""
Sentiment model VALIDATION / comparison on LABELLED data (the convincing benchmark).

Why: comparing VADER vs RoBERTa by agreement alone does not say which is correct.
Here every model is evaluated against ground-truth labels (star ratings of the
"Women's E-Commerce Clothing Reviews" corpus), reporting accuracy and macro-F1 —
the same logic as the supervisor's classification_report comparison (Session 4).

Models compared:
  - VADER                (lexicon baseline, no training)
  - SVM / Naive Bayes / Logistic Regression on TF-IDF  (classic ML, the prof's approach)
  - RoBERTa Cardiff      (transformer champion, zero-shot)

Gold labels : rating <= 2 -> negative (0); == 3 -> neutral (1); >= 4 -> positive (2)
Output      : results/sentiment_model_comparison.pdf  + printed classification reports

Run:
  pip install transformers datasets scikit-learn nltk matplotlib torch
  python validate_sentiment.py            (downloads the labelled dataset automatically)
"""
...
N_TOTAL = 8000     # balanced sample size (CPU-friendly)
TEST_SIZE = 0.25
...
def roberta_labels(texts):
    ...
    for i in range(0, len(texts), 64):
        ...
        print(f"    RoBERTa {min(i + 64, len(texts))}/{len(texts)}")
    return np.array(out)
...
def main():
    ...
    df = load_labelled()
    print(f"Labelled sample: {len(df)} | balance {df['label'].value_counts().sort_index().to_dict()}")
    train_df, test_df = train_test_split(df, test_size=TEST_SIZE, random_state=42,
                                         stratify=df["label"])
    y_test = test_df["label"].values

    # classic ML on TF-IDF (the prof's approach)
    ...
    # lexicon + transformer (no training, evaluated on the same test set)
    texts = test_df["text"].tolist()
    print("  Scoring VADER...")
    results["VADER"] = vader_labels(texts)
    print("  Scoring RoBERTa (Cardiff)...")
    results["RoBERTa (champion)"] = roberta_labels(texts)
    ...
    deployable = {"VADER", "RoBERTa (champion)"}     # usable on UNLABELLED Zara comments
    res["deployable"] = res["model"].isin(deployable)
    # supervised in-domain models on the left, zero-shot (deployable) on the right
    res = res.sort_values(["deployable", "macro_f1"],
                          ascending=[True, True]).reset_index(drop=True)
    res.to_csv(os.path.join(OUT_DIR, "sentiment_model_comparison.csv"), index=False)

    # comparison bar chart — two clearly-separated groups
    ...
    if n_sup:
        ax.text((n_sup - 1) / 2, 0.97,
                'Supervised, trained on this dataset\n(in-domain reference — NOT usable on unlabelled Zara comments)',
                ha='center', va='top', fontsize=8.5, color='#444')
    if zidx:
        ax.text((min(zidx) + max(zidx)) / 2, 0.97,
                'Zero-shot\n(usable on Zara comments)',
                ha='center', va='top', fontsize=8.5, color='#993c1d')
    ax.set_xticks(x); ax.set_xticklabels(res["model"], rotation=20, ha='right', fontsize=9)
    ax.set_ylabel('Score'); ax.set_ylim(0, 1.05)
    ax.set_title('Sentiment model validation on labelled fashion reviews (gold = star rating)\n'
                 'Among models deployable on the unlabelled corpus, RoBERTa > VADER',
                 fontsize=11)
    ax.legend(loc='upper right'); ax.grid(axis='y', alpha=0.3, linestyle='--', linewidth=0.5)
    plt.tight_layout()
    out = os.path.join(OUT_DIR, 'sentiment_model_comparison.pdf')
